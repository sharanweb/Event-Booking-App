...
router.patch("/:id",async(req,res)=>{
    try{
        let event = await Event.findByIdAndUpdate(req.params.id,req.body,{new:true}).lean().exec();
...
router.delete("/:id",async(req,res)=>{
    try{
        let event = await Event.findByIdAndDelete(req.params.id,req.body,{new:true}).lean().exec();
